...
socket.on("message_sent", function(user_id, message) {
	...
	//PIN PICTURE
	else if (message[1][0]=="pin"){
		pinPic(message[1][1], message[1][2], message[1][3], message[1][4]);
	}
	
	else if (message[1][0]=="movePin" & user_id!=my_user_id){
		idx=message[1][3];
		paper.pins[idx].translate(message[1][1], message[1][2]);
	}
...
function setToolBar(titleBarDiv,paper){
	...
	var y=10;
	var text="Pin Pic";
	but4=makeButton(x,y,text,paper2);		
	but4.click(function () {
			setMouseFunctions("pin");
			prevBut=but4;
	});
}
...
function sendPin(event){
	var picUrl=prompt("Insert url of image","");
		if (picUrl!=null){
			send_message(["pin",event.originalEvent.x,event.originalEvent.y-50,picUrl,""]);
		}
}

/*******************************************/

function pinPic(x,y,picURL,userId){
	/*
	var backgroundDiv=document.getElementById("backgroundDiv");
	var pinDiv= document.createElement('div');
	backgroundDiv.appendChild(pinDiv);

	pinDiv.style.position="absolute";
	pinDiv.style.left=x-280+'px';
	pinDiv.style.top=y-50+'px';
	pinDiv.style.height=100+'px';
	pinDiv.style.width=100+'px';
	...
	*/
	var img = new Image();
    img.onload = function() {  	
        //pinDiv.style.backgroundImage = "url(" + this.src + ")";
    	//pinDiv.style.backgroundImage =img
    	//pinDiv.style.backgroundSize="contain";
		//pinDiv.style.backgroundRepeat= "no-repeat";  
	...
	var st = paper.set();
  	...
  	var img2 = paper.image(picURL, x-280, y, 80, 80);
	var pin=paper.circle(x-280,y,10).attr({'fill':'orange'});
	pin.display=1;
	...
	st.mousedown(function () {
		st.oldX=st[0].attr('cx');
		st.oldY=st[0].attr('cy');
	});
	
	st.mouseup(function () {
		st.animate({opacity: 1}, 500, ">");
		dx=st[0].attr('cx')-st.oldX;
		dy=st[0].attr('cy')-st.oldY;
		send_message(["movePin",dx,dy,st.idx]);
	});
	
	
	st.idx=paper.pins.length;
    paper.pins[paper.pins.length]=st;
...
window.onload = function() {
	...
	//SET TRANSPARENT RECTANGLE OVERLAY TO RECEIVE EVENTS
	rect = paper.rect(5,55,1100,500-60);
	rect.attr("opacity",0);
